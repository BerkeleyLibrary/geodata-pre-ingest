...
vector_results = os.path.join(vector_process_path,"Results")
vector_csv = os.path.join(vector_results,par.RESULT_DIRECTORY_PARENTS[0],par.RESULT_DIRECTORY[3])

# Just a list of output directoies, they are generated by runing by "v_test_0_1_create_resultspace.py"
# vector_map = os.path.join(vector_results,par.RESULT_DIRECTORY_PARENTS[0],par.RESULT_DIRECTORY[1])
# vector_download = os.path.join(vector_results,par.RESULT_DIRECTORY_PARENTS[0],par.RESULT_DIRECTORY[0])
# vector_19139 = os.path.join(vector_results,par.RESULT_DIRECTORY_PARENTS[0],par.RESULT_DIRECTORY[4])
...
#  Fixture: 1) files under vector_original_source; 2) vector_ark_file ;
...
class MoveSourceDataToWorkspaceTestCase(unittest.TestCase):
    def setUp(self):
        dirs = [vector_work,vector_source,vector_csv]
        empty_result_dir(dirs)
        self.geoworkspace = geodata_workspace.GeodataWorkspace(vector_original_source,vector_process_path,geo_ext)

    def runTest(self):
        self.geoworkspace.excute()
        shp_file = os.path.join(vector_work,"1950prj_city","1950prj_city.shp")
        is_file = os.path.isfile(shp_file)
        self.assertTrue(is_file)

# Assign arks to geofiles under work, source directories
class AllocateArkTestCase(unittest.TestCase):
    def setUp(self):
        self.assign_arks = assign_arks.AssignArks(vector_process_path,vector_ark_file,geo_ext)

    def runTest(self):
        def has_ark_file():
            existed = False
            files_from_work = geo_helper.GeoHelper.files(vector_work)
...
#3) Generate 4 CSV Files under vector_csv
class ExportCsvTestCase(unittest.TestCase):
    def setUp(self):
        ...
        workspace_batch_path = geo_helper.GeoHelper.work_path(vector_process_path)
        dest_csv_files = geo_helper.GeoHelper.dest_csv_files(vector_process_path)
...
vector_export_work = os.path.join(vector_export_process_path,"Work")
# vector_export_source = os.path.join(vector_export_process_path,"Source")
vector_export_updated_csv = os.path.join(vector_export_results,par.RESULT_DIRECTORY_PARENTS[0],par.RESULT_DIRECTORY[5])

# output directories
vector_export_map = os.path.join(vector_export_results,par.RESULT_DIRECTORY_PARENTS[0],par.RESULT_DIRECTORY[1])
vector_export_download = os.path.join(vector_export_results,par.RESULT_DIRECTORY_PARENTS[0],par.RESULT_DIRECTORY[0])
vector_export_19139 = os.path.join(vector_export_results,par.RESULT_DIRECTORY_PARENTS[0],par.RESULT_DIRECTORY[4])
...
class ValidateCsvTestCase(unittest.TestCase):
    def setUp(self):
        self.v_csv = validate_csv.ValidateCSV(vector_export_process_path)
    def runTest(self):
...
class TranformISO19139TestCase(unittest.TestCase):
    def setUp(self):
        ...
        valid_updated_csv = validate_csv.ValidateCSV(vector_export_process_path)
        if valid_updated_csv.updated_csv_files_existed():
            if valid_updated_csv.updated_csv_files_valid():
                updated_csv_files = geo_helper.GeoHelper.dest_csv_files_updated(vector_export_process_path)
                csv_collection = csv_iso_collection.CsvIsoCollection(updated_csv_files).csv_collection()
                self.csvtransform = csv_tranform.CsvTransform(csv_collection,vector_export_process_path)
    def runTest(self):
        self.csvtransform.transform_iso19139()
        iso19139_file = os.path.join(vector_export_19139,"s7w117","iso19139.xml")
        is_file = os.path.isfile(iso19139_file)
        self.assertTrue(is_file)

class TranformGeoblacklightTestCase(unittest.TestCase):
    def setUp(self):
        dirs = [vector_export_geoblacklight]
        empty_result_dir(dirs)
        self.csvtransform = None
        valid_updated_csv = validate_csv.ValidateCSV(vector_export_process_path)
        if valid_updated_csv.updated_csv_files_existed():
    ...
    def runTest(self):
        ...
        geoblacklight_file = os.path.join(vector_export_geoblacklight,"s7w117","geoblacklight.json")
        is_file = os.path.isfile(geoblacklight_file)
        self.assertTrue(is_file)

class MapZipTestCase(unittest.TestCase):
    def setUp(self):
        dirs = [vector_export_map]
        empty_result_dir(dirs)

        self.zip_data = None
        valid_updated_csv = validate_csv.ValidateCSV(vector_export_process_path)
        if valid_updated_csv.work_files_existed():
            workspace_batch_path = geo_helper.GeoHelper.work_path(vector_export_process_path)
            projected_map_path = geo_helper.GeoHelper.map_download_path(vector_export_process_path)
            self.zip_data = zip_data.ZipData(workspace_batch_path,projected_map_path,"map.zip",par.vector_exts)

    def runTest(self):
        self.zip_data.map_zipfiles()
        map_zip_file = os.path.join(vector_export_map,"s7w117","map.zip")
...
class DownloadZipTestCase(unittest.TestCase):  # Attention: need second workspace for original data not under it's directory
    def setUp(self):
        dirs = [vector_export_download]
        empty_result_dir(dirs)

        self.zip_data = None
        valid_updated_csv = validate_csv.ValidateCSV(vector_export_process_path)
        if valid_updated_csv.source_files_existed():
            sourcedata_batch_path = geo_helper.GeoHelper.source_path(vector_export_process_path)
            data_download_path = geo_helper.GeoHelper.data_download_path(vector_export_process_path)
            self.zip_data = zip_data.ZipData(sourcedata_batch_path,data_download_path,"data.zip")
    def runTest(self):
        self.zip_data.download_zipfiles()
        data_zip_file = os.path.join(vector_export_download,"s7w117","data.zip")
        is_file = os.path.isfile(data_zip_file )
        self.assertTrue(is_file)

class MerritTestCase(unittest.TestCase):
    def setUp(self):
        dirs = [vector_export_merritt]
        empty_result_dir(dirs)
        ...
        if valid_updated_csv.geoblacklight_files_existed():
            if valid_updated_csv.data_zip_files_existed():
                final_result_path =  geo_helper.GeoHelper.final_result_path(vector_export_process_path)
                self.merritt_collection = merritt.Merrit(vector_export_process_path,final_result_path)
    def runTest(self):
        self.merritt_collection.save_merritt_to_file()
        merritt_file = os.path.join(vector_export_merritt,"merritt.txt")
